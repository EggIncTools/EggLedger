        ...
        return AuthentikAspNetAuth.AddIfConfigured(builder, new AuthentikAspNetAuthOptions {
            ...
            OnResolved = async (result, identity, ctx) => {
                ...
                try {
                    ...
                    }, DbRetry, ct: ctx.RequestAborted);
                } catch (Exception ex) when (ex is not OperationCanceledException) {
                    ctx.RequestServices.GetRequiredService<ILoggerFactory>()
                        .CreateLogger(nameof(AuthentikAuth))
                        .LogWarning(ex, "auth: failed to upsert users row for {UserId}", result.UserId);
                }
            },
        });
    }
